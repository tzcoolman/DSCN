...
def load_target(filename,target_list,nx_graph):
	...
	f1.close()
	for ele1 in temp_list:
		for ele2 in temp_list:
			if ele1!=ele2:
				target_list[ele1+"\t"+ele2]=1
...
def add_weight_to_sub_network(graph,exp_hash,essen_hash):
	for node in graph.nodes():
		graph.node[node]['weight']=essen_hash[node]
	for edge in graph.edges():
		#print (exp_hash[edge[0]])
		#print (np.array(exp_hash[edge[0]]))
		#a=np.array(list(exp_hash[edge[0]]))
		#b=np.array(list(exp_hash[edge[1]]))
		#print (len(a),len(b))
		graph[edge[0]][edge[1]]['weight']=np.corrcoef(np.array(list(exp_hash[edge[0]])),np.array(list(exp_hash[edge[1]])))[0,1]
		#graph.add_edge(temp[0],temp[1],weight=np.corrcoef(np.array(exp_hash[edge[0]]),np.array(exp_hash[edge[1]]))[0,1])
	#for cluster in cluster_list.values()#        temp_graph=graph.subgraph(cluster)
