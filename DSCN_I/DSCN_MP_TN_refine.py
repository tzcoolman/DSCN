...
def new_essentiality(sample_list,essen_hash,graph):
	for node in graph.nodes():
		if sample_list==[]:
			graph.node[node]['weight_new']=graph.node[node]['weight']
		else:
			temp_list=sample_by_num(sample_list,essen_hash[node])
		#print node," : ",temp_list," new essentiality"
		#print essen_hash[node]
			graph.node[node]['weight_new']=np.mean(temp_list) #get new average essentiality value for each node in the subnetwork
############################################################
#load exp, gene essen and PPI
############################################################
target_file="FDA_drug_target.txt.overlap"
#target_list={}
#DSCN_load.load_target(target_file,target_list)

exp_file="TNBC_CCLE.recali.uniq.overlap"
essen_file="HS578T_CRISPR.txt.uniq.overlap"
ppi_file="string_PPI.network.overlap"
cluster_file="TCGA_TNBC_WG.cluster"
...
############################################################
	...
	new_essentiality([],essen_hash,SG_1)
...
#############################################################
			...
			#print np.mean(temp_total_list)
			#print np.std(temp_total_list)
			#print len(temp_total_list)
			#print bg_mu,bg_std,bg_count
			P_value=DSCN_p.calculate_p(np.mean(temp_total_list),np.std(temp_total_list),len(temp_total_list),bg_mu,bg_std,bg_count)
#############################################################
###score 2nd target
#############################################################
			result+="T2: "+each_target1+" AND "+each_target2+": "+str(temp_score+temp_score2)+"\t"+str(p_value)+"\n"
#############################################################
#return result
#############################################################
	return result[:-1]
...
if __name__ == '__main__':
	...
	for each_chunk in total_result:
		print "\n".join(total_result)
'''
def test_execute(test_set):
	print test_set[0],test_set[1]
